#!/usr/bin/env python3
"""
Unified GeMap Evaluation Script with NORMALIZED Camera Parameters
===================================================================
Runs GeMap inference with configurable camera inputs AND evaluates with camera-specific FOV clipping.

**KEY MODIFICATION**: Camera-to-ego translations are NORMALIZED to mimic VGGT's non-metric parameters.

This script modifies the camera2ego translation parameters to remove absolute metric scale,
simulating what would happen if camera extrinsics were estimated from VGGT (which outputs 
relative, non-metric camera poses). The normalization:

1. Preserves relative geometry (directions between cameras)
2. Preserves approximate relative distances (ratios)
3. Removes absolute metric scale (meters → unitless ~1.0)

This allows evaluating how GeMap performs when camera calibration is NOT available in metric scale,
which is the key advantage of VGGT.

Usage Examples:
---------------
# Front camera only with normalized parameters
python tools/gemap_eval_unified_normalized.py --cameras CAM_FRONT

# Front + Back cameras with normalized parameters
python tools/gemap_eval_unified_normalized.py --cameras CAM_FRONT CAM_BACK

# All 6 cameras (baseline) with normalized parameters
python tools/gemap_eval_unified_normalized.py --cameras all
"""
...
def normalize_camera_translations(img_metas_list: List[Dict], active_camera_indices: List[int] = None, logger=None) -> List[Dict]:
    """
    Normalize camera2ego translations per timestamp to mimic VGGT's non-metric parameters.
    
    **CRITICAL**: Only normalizes ACTIVE cameras to ensure consistent scaling.
    
    Strategy:
    1. For each timestamp, collect camera translations for ACTIVE cameras only
    2. Compute the mean and std of translation magnitudes across ACTIVE cameras
    3. Normalize each ACTIVE camera translation to have unit norm (direction preserved)
    4. Scale normalized translations to have mean=1.0, std~original relative spread
    5. Keep INACTIVE cameras unchanged (will be zeroed out anyway)
    
    This preserves:
    - Relative geometry (directions between cameras)
    - Approximate relative distances (ratios) WITHIN ACTIVE SET
    
    But removes:
    - Absolute metric scale (meters)
    
    Args:
        img_metas_list: List of img_metas dicts, one per timestamp in sequence
        active_camera_indices: List of camera indices that are actually used (e.g., [0, 3] for FRONT+BACK)
                              If None, normalizes all 6 cameras
        logger: Optional logger for debugging
    
    Returns:
        Modified img_metas_list with normalized camera2ego translations for ACTIVE cameras
    """
    if active_camera_indices is None:
        active_camera_indices = list(range(6))  # Default: all cameras
    
    if logger:
        logger.info("\n" + "="*80)
        logger.info("Normalizing Camera Translations (VGGT-style non-metric)")
    ...
    for t_idx, img_meta in enumerate(img_metas_list):
        # Check if camera2ego exists
        if 'camera2ego' not in img_meta:
            if logger and t_idx == 0:
                logger.warning("'camera2ego' not found in img_metas. Skipping normalization.")
            continue
        ...
        for cam_idx, cam2ego in enumerate(camera2ego_list):
            if isinstance(cam2ego, np.ndarray):
                trans = cam2ego[:3, 3].copy()
            elif torch.is_tensor(cam2ego):
                trans = cam2ego[:3, 3].cpu().numpy().copy()
            else:
                trans = np.array(cam2ego)[:3, 3].copy()
            original_translations.append(trans)
        
        original_translations = np.array(original_translations)  # Shape: [6, 3]
        
        # Compute norms (distances from ego) ONLY for ACTIVE cameras
        norms = np.linalg.norm(original_translations, axis=1)  # Shape: [6]
        active_norms = norms[active_camera_indices]  # Only active cameras
        
        # Log original values for first timestamp
        if logger and t_idx == 0:
            logger.info(f"\nTimestamp {t_idx} - Original camera translations (meters):")
            cam_names = list(CAMERA_MAP.keys())
            logger.info("  ALL cameras (for reference):")
            for cam_idx, (trans, norm) in enumerate(zip(original_translations, norms)):
                active_marker = " [ACTIVE]" if cam_idx in active_camera_indices else " [INACTIVE]"
                logger.info(f"    {cam_names[cam_idx]:15s}: [{trans[0]:6.2f}, {trans[1]:6.2f}, {trans[2]:6.2f}] norm={norm:.3f}m{active_marker}")
            logger.info(f"  ACTIVE cameras only:")
            logger.info(f"    Mean norm: {active_norms.mean():.3f}m, Std: {active_norms.std():.3f}m")
        
        # Normalize to unit vectors (preserve direction) - ONLY for ACTIVE cameras
        # Handle zero-norm case (camera at ego origin)
        normalized_translations = original_translations.copy()  # Start with original
        for i in active_camera_indices:  # Only normalize active cameras
            if norms[i] > 1e-6:
                normalized_translations[i] = original_translations[i] / norms[i]
            # else: keep original (zero vector)
        
        # Scale to have mean norm = 1.0, preserving relative spread - ONLY for ACTIVE cameras
        # This makes translations "approximately" 1 unit from ego, mimicking non-metric scale
        mean_norm = active_norms.mean()  # Mean of ACTIVE cameras only
        if mean_norm > 1e-6:
            # Preserve relative distances: scale each ACTIVE camera by (original_norm / mean_norm)
            relative_scales = norms / mean_norm  # Compute for all, but only apply to active
            for i in active_camera_indices:  # Only scale active cameras
                normalized_translations[i] *= relative_scales[i]
        
        # Log normalized values for first timestamp
        if logger and t_idx == 0:
            normalized_norms = np.linalg.norm(normalized_translations, axis=1)
            active_normalized_norms = normalized_norms[active_camera_indices]
            logger.info(f"\nTimestamp {t_idx} - Normalized camera translations (unitless):")
            logger.info("  ACTIVE cameras (normalized):")
            for cam_idx in active_camera_indices:
                trans = normalized_translations[cam_idx]
                norm = normalized_norms[cam_idx]
                logger.info(f"    {cam_names[cam_idx]:15s}: [{trans[0]:6.3f}, {trans[1]:6.3f}, {trans[2]:6.3f}] norm={norm:.3f}")
            logger.info(f"    Mean norm: {active_normalized_norms.mean():.3f}, Std: {active_normalized_norms.std():.3f}")
            logger.info(f"    Scale reduction: {mean_norm:.3f}x (from ~{mean_norm:.1f}m to ~{active_normalized_norms.mean():.1f})")
            if len(active_camera_indices) < 6:
                logger.info("  INACTIVE cameras (unchanged - will be zeroed):")
                for cam_idx in range(6):
                    if cam_idx not in active_camera_indices:
                        trans = normalized_translations[cam_idx]
                        norm = normalized_norms[cam_idx]
                        logger.info(f"    {cam_names[cam_idx]:15s}: [{trans[0]:6.2f}, {trans[1]:6.2f}, {trans[2]:6.2f}] norm={norm:.3f}m")
        
        # Update camera2ego matrices with normalized translations
        for cam_idx in range(6):
            if isinstance(camera2ego_list[cam_idx], np.ndarray):
                camera2ego_list[cam_idx][:3, 3] = normalized_translations[cam_idx]
            elif torch.is_tensor(camera2ego_list[cam_idx]):
                camera2ego_list[cam_idx][:3, 3] = torch.from_numpy(normalized_translations[cam_idx]).to(
                    camera2ego_list[cam_idx].device
                ).to(camera2ego_list[cam_idx].dtype)
            else:
                # Assume list or similar mutable structure
                camera2ego_list[cam_idx][:3, 3] = normalized_translations[cam_idx]
        
        # Also update lidar2img if it exists (it depends on camera2ego)
        # lidar2img = intrinsic @ cam2ego^-1 @ lidar2ego
        # Since we changed cam2ego, we need to recompute lidar2img
        if 'lidar2img' in img_meta and 'camera_intrinsics' in img_meta and 'lidar2ego' in img_meta:
            ...
            for cam_idx in range(6):
                ...
                # Recompute lidar2img: lidar2img = intrinsic @ ego2cam @ lidar2ego
                # where ego2cam = inv(cam2ego)
    ...
    if logger:
        logger.info("✓ Camera translation normalization complete")
        logger.info("="*80 + "\n")
...
def run_gemap_inference(
    ...
    Args:
        active_camera_indices: Indices of cameras to use for normalization.
                              If None, uses camera_indices.
    """
    print("\n" + "="*80)
    ...
    for i, data in enumerate(data_loader):
        try:
            ...
            # NORMALIZE CAMERA TRANSLATIONS (VGGT-style non-metric)
            # Apply normalization to mimic VGGT output where camera parameters are not in metric scale
            # CRITICAL: Only normalize ACTIVE cameras to ensure consistent scaling
            img_metas = normalize_camera_translations(
                img_metas if isinstance(img_metas, list) else [img_metas],
                active_camera_indices=camera_indices,  # Only normalize cameras we're actually using
  ...
  python %(prog)s --no-clipping --cameras CAM_FRONT
    ...
    if not args.skip_inference:
        predictions_pkl = run_gemap_inference(
            ...
            active_camera_indices=camera_indices  # Pass active cameras for normalization
        )
    else:
        print("\n" + "="*80)
